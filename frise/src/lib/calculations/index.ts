 ...
 * Based on exponential functions from Borbély's Two-Process Model.
 * 
 * @param hoursAwake - Hours since waking up
 * @param sleepDurationHours - How long the person slept
 * @returns Sleep pressure value (higher = more tired)
 */
export function calculateProcessS(hoursAwake: number, sleepDurationHours: number): number {
  // Initial sleep pressure after waking depends on sleep duration
  // More sleep = lower initial pressure
  const idealSleepHours = 8;
  const sleepDebtFactor = Math.max(0, (idealSleepHours - sleepDurationHours) / idealSleepHours);
  const initialPressure = 0.1 + (sleepDebtFactor * 0.3); // Range: 0.1 - 0.4
  
  // Sleep pressure builds up exponentially during waking
  // S(t) = S_upper - (S_upper - S_initial) * e^(-t/τ_w)
  const upperLimit = 1.0;
  const pressure = upperLimit - (upperLimit - initialPressure) * Math.exp(-hoursAwake / WAKE_PRESSURE_TIME_CONSTANT);
 ...
 * 
 * The circadian alertness signal follows a roughly sinusoidal pattern
 * with a period of ~24 hours. Peak alertness is typically mid-morning
 * and mid-afternoon, with dips in early afternoon and at night.
 * 
 * @param hourOfDay - Hour of the day (0-23)
 * @returns Circadian alertness value (0-1, higher = more alert)
 */
export function calculateProcessC(hourOfDay: number): number {
  // Normalize hour to handle hours > 24 (for next day)
  const normalizedHour = hourOfDay % 24;
  
  // Phase shift: peak alertness around 10 AM and 6 PM
  // Lowest around 3-4 AM and 2-3 PM (post-lunch dip)
  const phaseShift = -10; // Shift to align peak with ~10 AM
  
  // Main circadian component (24-hour cycle)
  const mainCycle = Math.cos(2 * Math.PI * (normalizedHour + phaseShift) / TAU);
  
  // Add a secondary component for the post-lunch dip (12-hour harmonic)
  // This creates a smaller dip in the early afternoon
  const secondaryCycle = 0.25 * Math.cos(4 * Math.PI * (normalizedHour + phaseShift) / TAU);
  
  // Combine and normalize to 0-1 range
  const combined = (mainCycle + secondaryCycle) / 1.25;
 ...
 * 
 * Productivity = Circadian alertness - Sleep pressure
...
export function calculateProductivity(
  ...
  const processC = calculateProcessC(hourOfDay);
  const processS = calculateProcessS(hoursAwake, sleepDurationHours);
  
  // Productivity is circadian alertness reduced by sleep pressure
  // Weight circadian more heavily as it has more pronounced effects
  const rawProductivity = (processC * 0.6) + ((1 - processS) * 0.4);
  
  // Scale to 0-100 and apply non-linear scaling for better distribution
  const scaled = Math.pow(rawProductivity, 0.8) * 100;
